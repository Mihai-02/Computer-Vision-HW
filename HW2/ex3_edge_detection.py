...
def detect_edges(init_image):
    ...
    init_image = np.dot(init_image[...,:3], [0.2989, 0.5870, 0.1140])
    ...
    for a in range(2):
        convoluted = np.zeros((y_img_s-2*h, x_img_s-2*w))
        ...
        for i in range(h, y_img_s-h):
            for j in range(w, x_img_s-w):
                ...
                #         convoluted[i-h][j-w] = convoluted[i-h][j-w] + kernel[y_filter_size-m-1][x_filter_size-n-1]*init_image[i-h+m][j-w+n]
...
filtered_image = detect_edges(image)
im = Image.fromarray(filtered_image, 'L')
im.save("Demo/010_edges.jpeg")
